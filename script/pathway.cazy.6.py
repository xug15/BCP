...
#output1=output+'.test.xgboost_testset.png'
#plt.savefig('../data/mloutput/cazy.xgboost_testset.png')
plt.savefig(output+'.xgboost_testset.png')
plt.show()
...
plt.savefig(output+'.xgboost_fullset.png')
plt.show()
...
plt.savefig(output+'.xgboost_summary.png')
...
output7=output+'.XGBoost_Gain_2.png'
fig.savefig(output7, bbox_inches='tight', pad_inches = 0, dpi=200)
plt.show()
...
gainlist.loc[gainlist['gene'].isin(gainlist_subset_list)].to_csv(output+'.gain_selection.csv') # 将gainlist这个dataframe中大于增益值平均值的基因名所在的所有行都输出到gain_selection.csv文件中
...
plt.savefig(output+'.gain_testset.png')
plt.show()
...
plt.savefig(output+'.gain_fullset.png')
plt.show()
...
plt.savefig(output+'.gain_summary.png')
...
fig.savefig(output+'.Perm_Importance_2.png', bbox_inches='tight', pad_inches = 0, dpi=200)
plt.show()
...
permlist.loc[permlist['gene'].isin(permlist_subset)].to_csv(output+'.perm_selection.csv')
...
filename = output+'.model_2022-09-12.sav'
...
plt.savefig(output+'.perm_testset.png')
plt.show()
...
plt.savefig(output+'.perm_fullset.png')
plt.show()
...
test_set_graph_perm.to_csv(output+'.perm_test_graph.csv')
...
full_set_graph_perm.to_csv(output+'.perm_full_graph.csv')
...
plt.savefig(output+'.perm_summary.png')
...
output2=output+'.autoencoder.png'
plot_model(model, output2, show_shapes=True)
...
output3=output+'.encoder.png'
plot_model(encoder, output3, show_shapes=True)
...
plt.xlabel("Permutation Importance")
#fig.savefig('../data/mloutput/cazy.Perm_Importance_2.png', bbox_inches='tight', pad_inches = 0, dpi=200)
output4=output+'.Perm_Importance_2.png'
fig.savefig(output4, bbox_inches='tight', pad_inches = 0, dpi=200)
plt.show()
...
lle_permlist.loc[lle_permlist['PC'].isin(lle_perm_importance)].to_csv(output+'.lle_selection.csv')
...
plt.xlabel("Permutation Importance")
#fig.savefig('../data/mloutput/cazy.Perm_Importance_2.png', bbox_inches='tight', pad_inches = 0, dpi=200)
output5=output+'.Perm_Importance_2.png'
fig.savefig(output5, bbox_inches='tight', pad_inches = 0, dpi=200)
plt.show()
...
pca_permlist.loc[pca_permlist['PC'].isin(pca_permlist_subset_list)].to_csv(output+'.pca_selection.csv')
...
X_pca_br.to_csv(output+'.wtf.csv', index=False)
X_pca_br = pd.read_csv(output+'.wtf.csv')
